...
def step_forward(
    ...
    x_n = copy.deepcopy(x)
    [mu_lambda, mu_lambda_self] = BarrierEnergy.compute_mu_lambda(
        x, n, o, bp, be, contact_area, mu
    )  # compute mu * lambda for each node using x^n
    ...
    while (LA.norm(p, inf) / h > tol) | (
        sum(DBC_satisfied) != len(DBC)
    ):  # also check whether all DBCs are satisfied
        print("Iteration", iter, ":")
        print("residual =", LA.norm(p, inf) / h)

        if (LA.norm(p, inf) / h <= tol) & (sum(DBC_satisfied) != len(DBC)):
            # increase DBC stiffness and recompute potential value record
            DBC_stiff[0] *= 2
            E_last = IP_val(
                x,
                e,
        ...
        # filter line search
        alpha = min(
            BarrierEnergy.init_step_size(x, n, o, bp, be, p),
            NeoHookeanEnergy.init_step_size(x, e, p),
        )  # avoid interpenetration, tunneling, and inversion
        while (
            IP_val(
                x + alpha * p,
                e,
...
def IP_grad(
    ...
    return (
        InertiaEnergy.grad(x, x_tilde, m)
        + h
        * h
        * (  # implicit Euler
...
def search_dir(
    ...
    for i, j in zip(*projected_hess.nonzero()):
        if (is_DBC[int(i / 2)] & DBC_satisfied[int(i / 2)]) | (
            is_DBC[int(j / 2)] & DBC_satisfied[int(j / 2)]
        ):
